  ...
  while(true) {  //break 걸릴때 까지 계속 반복 한다
        ...
        String[] str=email.split("@");  //id를 출력하기 위해서 split으로 @기준으로 분리한다.
          if(Pattern.matches(Idlist,str[0])) {  //@전 id가 id정규식과 같다면
             if(email.endsWith(".com") ||email.endsWith(".co.kr") || email.endsWith(".net") ||email.endsWith(".org")) {  //끝나는 지점이 메일주소로 끝나야 한다.
                String[] arry=email.split("@|\\.");  //메일 서버가 있어야 함
                   if(arry[1].isBlank()) {    //공백으로 이루어져 있다면
                      System.out.println("메일 서버가 있어야 합니다.");
                   }else {
                      break;
                   }                
              }else {
                 System.out.println(".com , .co.kr , .net, .org 중 하나로 끝내야 합니다. 다시입력하세요");                
              }
        }
          else { //id 정규식과 같지 않다면
           System.out.println("id형식이 다릅니다. 다시 입력하세요");
                }                    
  }else {
     ...
     String sttt=password.substring(0,2)+"*".repeat(password.length()-2);  
     System.out.println("PASSWORD:"+sttt);        
  }
}
